...
pub const CENSUS: [(&str, &str); 19] = [
    ...
    ("project", "primary"),
    ("project", "worktrees"),
    // Which store this project's work graph lives in: the name of a store
    // adapter an installed pack carries, or an absolute path to an adapter
    // executable (docs/store.md).
    ("store", "adapter"),
];
