//! `fleet agent schema` through the shipped binary: the agent contract's JSON
...
const COMMITTED: &str = include_str!("../../core/src/agent/agent.schema.json");

/// The agent's documentation, whose verbs table names the verbs.
...
impl Machine {
    fn new(label: &str) -> Machine {
        ...
        let root = std::env::temp_dir().join(format!(
            "fleet-cli-agent-schema-{label}-{}-{n}",
            std::process::id()
        ));
...
/// The first backticked word of each row of docs/agent.md's verbs table: the
/// verbs the contract names.
fn documented_verbs() -> Vec<String> {
    let table = AGENT_DOC
        .split("\n## Verbs\n")
        .nth(1)
        .expect("docs/agent.md has a Verbs section");
...
fn schema_prints_the_committed_document_and_exits_zero() {
    ...
    let out = machine.run(&["agent", "schema"]);
    assert_eq!(out.status.code(), Some(0), "{out:?}");
    assert!(out.stderr.is_empty(), "{out:?}");
    assert!(
        String::from_utf8_lossy(&out.stdout) == COMMITTED,
        "fleet agent schema does not print core/src/agent/agent.schema.json — run `cargo run -p \
         fleet-cli -- agent schema > core/src/agent/agent.schema.json`"
    );
}

/// `.verbs | keys` is exactly the verbs docs/agent.md's table names, and
/// `."$defs".Posture.enum` is the three postures.
///
